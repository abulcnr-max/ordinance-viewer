...
Streamlit app — reads classified JSON files from Google Drive
"""
...
def load_from_gdrive(folder_id):
    """Load all JSON files from Google Drive folder."""
    try:
        # Use API key for public folders
        api_key = st.secrets.get("GDRIVE_API_KEY", os.getenv("GDRIVE_API_KEY", ""))
        ...
        for idx, file in enumerate(files):
            try:
                ...
                county_town_map[county].append((town, file["name"]))
        ...
        st.error(f"Failed to load from Google Drive: {e}")
        return {}, {}


# ── Fallback: load from local folder ──
@st.cache_data
def load_from_local(folder="./data/classified"):
    """Load JSON files from local folder (fallback)."""
    p = Path(folder)
    if not p.exists():
        return {}, {}
...
if GDRIVE_FOLDER_ID != "YOUR_FOLDER_ID_HERE":
    county_town_map, all_data = load_from_gdrive(GDRIVE_FOLDER_ID)
else:
    county_town_map, all_data = load_from_local()
...
with st.sidebar:
    ...
    if not county_town_map:
        st.error("No data found. Check your Google Drive folder ID or local data folder.")
        st.stop()

    # County selector — default to Atlantic County
    ...
    # Town selector — default to Hammonton
